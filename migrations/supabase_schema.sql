...
-- 公开访问策略（匿名用户可以读写）
CREATE POLICY "Allow all for anon users" ON players FOR ALL USING (true);
CREATE POLICY "Allow all for anon users" ON player_equipment FOR ALL USING (true);
CREATE POLICY "Allow all for anon users" ON player_items FOR ALL USING (true);
CREATE POLICY "Allow all for anon users" ON leaderboard FOR ALL USING (true);
CREATE POLICY "Allow all for anon users" ON player_achievements FOR ALL USING (true);
CREATE POLICY "Allow all for anon users" ON player_daily_challenges FOR ALL USING (true);
CREATE POLICY "Allow all for anon users" ON player_subscriptions FOR ALL USING (true);
CREATE POLICY "Allow all for anon users" ON subscription_orders FOR ALL USING (true);
...
INSERT INTO subscription_tiers (code, name, description, icon, color, price, price_monthly, duration_days, features, display_order) VALUES
('free', '免费用户', '基础游戏体验', '👤', '#888888', 0, 0, 0, '{"aiAdvice": 3, "safeboxSlots": 5, "extractProtection": false}', 0),
('basic', '基础会员', '解锁更多 AI 功能', '🥉', '#cd7f32', 1800, 1500, 30, '{"aiAdvice": 10, "safeboxSlots": 10, "extractProtection": true}', 1),
('premium', '高级会员', '完整 AI 战术体验', '🥈', '#c0c0c0', 5800, 4800, 30, '{"aiAdvice": -1, "safeboxSlots": 15, "extractProtection": true, "extraDropRate": 5}', 2),
('vip', 'VIP会员', '顶级特权体验', '🥇', '#ffd700', 12800, 9800, 30, '{"aiAdvice": -1, "safeboxSlots": 20, "extractProtection": true, "extraDropRate": 10, "prioritySupport": true}', 3)
...
INSERT INTO achievements (code, name, description, icon, category, condition, reward, display_order) VALUES
('first_extract', '首次撤离', '成功完成第一次撤离', '🚁', 'progress', '{"type": "extract", "count": 1}', '{"coins": 500}', 0),
('survivor_10', '幸存者', '累计成功撤离10次', '🏃', 'progress', '{"type": "extract", "count": 10}', '{"coins": 2000}', 1),
('killer_5', '猎人', '单局击杀5个敌人', '⚔️', 'combat', '{"type": "kills", "count": 5}', '{"coins": 1000}', 2),
('red_drop', '开红时刻', '首次获得红色品质装备', '💎', 'loot', '{"type": "red_quality", "count": 1}', '{"coins": 3000}', 3),
('millionaire', '百万富翁', '单局收益超过100000', '💰', 'wealth', '{"type": "profit", "value": 100000}', '{"coins": 10000}', 4)
ON CONFLICT (code) DO NOTHING;
...
INSERT INTO daily_challenges (code, name, description, icon, category, difficulty, condition, reward, display_order) VALUES
('daily_extract', '安全撤离', '完成一次成功撤离', '🚁', 'extract', 'easy', '{"type": "extract", "count": 1}', '{"coins": 300}', 0),
('daily_kill', '猎杀任务', '击杀3个敌人', '⚔️', 'combat', 'normal', '{"type": "kills", "count": 3}', '{"coins": 500}', 1),
('daily_loot', '搜刮专家', '获得5件装备', '📦', 'loot', 'easy', '{"type": "loot", "count": 5}', '{"coins": 400}', 2),
('daily_boss', '挑战黑区', '探索黑区实验室', '💀', 'explore', 'hard', '{"type": "zone_boss", "count": 1}', '{"coins": 1000}', 3)
ON CONFLICT (code) DO NOTHING;
